...
namespace SC3_Alarm_Module_WS.Code_Files.DBL
{
    public class DBL
    {
        #region Sql Con.
        ...
        {
            string SC3_Alarm_DB_Conn_Str = ConfigurationManager.ConnectionStrings["dbConnection"].ConnectionString;
            string[] SC3_Alarm_DB_Conn_Str_Parts = SC3_Alarm_DB_Conn_Str.Split(';');
            string SC3_Alarm_DB_Conn_Str_New = "";

            for (int i = 0; i < SC3_Alarm_DB_Conn_Str_Parts.Length; i++)
            {
                if (i != 1)
                {
                    SC3_Alarm_DB_Conn_Str_New += SC3_Alarm_DB_Conn_Str_Parts[i] + "; ";
                }
                else
                {
                    SC3_Alarm_DB_Conn_Str_New += "Database=" + database_Name + "; ";
                }
            }
            sqlServerConnection = new SqlConnection(SC3_Alarm_DB_Conn_Str_New);
        }
            ...
            try
            {
                ...
                if (Check_Parms_StP(Stored_Procedure, parms) == true)
                {
                    ...
                    for (int i = 0; i < parms.Length - (parms.Length / 2); i++)
                    {
                        ...
                        if (parms[i, 1] == null || parms[i, 1].ToString() == "")
                        {
                            obj = DBNull.Value;
                        }
                        else
                    ...
                    flag = sqlCmd.ExecuteNonQuery() > 0 ? true : false;
                    return flag;
                }
                else
                {
